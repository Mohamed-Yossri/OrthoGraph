...
## Case presentation update (2026-09-25)

A user re-uploaded a 16-bit OPG after the normalization fix. Case `9e4bb355` confirmed `source_mode=I;16` and produced 33 tooth candidates, four YOLO26 `third_molar` anatomy observations, and zero pathology candidates; an exploratory Liodon all-class pass on the saved image also produced zero candidates. Orientation remained unknown, so the old UI displayed question marks and an empty odontogram despite candidate FDI numbers being present. The UI now opens orientation setup automatically, shows candidate FDI labels with `?` and provisional odontogram slots, and explicitly separates pathology-candidate and anatomy-observation counts. These are presentation changes; no diagnostic claims or model thresholds changed. JavaScript syntax and 28 Python tests passed. Headless browser smoke was unavailable in this Studio because Chromium lacks `libatk-bridge-2.0.so.0`. Live static files were synced; full reload may be needed to fetch the new JavaScript.
